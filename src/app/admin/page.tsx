          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {/* Nếu đang rỗng (chưa có API), tôi mock 1 dòng để bạn xem giao diện */}
                  {(customers.length > 0 ? customers.filter(c => filterSearch(c.name || c.email)) : [
                    { id: "1", name: "Trần Văn Linh", email: "linh@gmail.com", phone: "0912345678", role: "USER", status: "ACTIVE" }
                  ]).map(customer => (
                    <tr key={customer.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Sửa thông tin"><Edit3 className="w-4 h-4"/></button>
                        <button className="p-2 text-red-600 hover:bg-red-50 rounded-lg ml-1" title="Khóa tài khoản (Ban)"><Trash2 className="w-4 h-4"/></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
