...
import {
  ...
  Search, Bell, LogOut, Plus, Edit3, Trash2, Eye, CheckCircle, DollarSign, Loader2, X
} from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Nếu trong file admin bạn có sẵn hàm showToast/alert thì dùng, ví dụ:
  ...
  // Bổ sung discountPrice vào State
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { setIsCatModalOpen(false); loadTabContextData(); }
  ...
  const openProductModal = (prod: any = null) => {
    if (prod) {
      ...
      setProdForm({
        ...
        discountPrice: prod.discountPrice ? String(prod.discountPrice) : "", // Nạp dữ liệu giá KM
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(url, {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...prodForm, sizes: sizesArray })
    });
    ...
    if (res.ok) {
      ...
      alert(errData.error || "Lỗi lưu trữ sản phẩm");
    }
  };
    ...
    switch (activeTab) {
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">Size hỗ trợ</th>
                    ...
                    <tr key={prod.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-1 flex-wrap w-32">
                          {prod.sizes?.slice(0, 3).map((s: string) => <span key={s} className="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] font-bold text-slate-600">{s}</span>)}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        {/* HIỂN THỊ GIÁ GỐC VÀ GIÁ KHUYẾN MÃI Ở LƯỚI ADMIN */}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                { label: "Đánh giá 5 sao", value: stats.fiveStarReviews || 0, icon: Star, color: "text-amber-600", bg: "bg-amber-50" }
              ].map((stat, idx) => {
      ...
      case "CUSTOMERS":
        return (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-slate-800">Quản lý Khách hàng & Tài khoản</h2>
            </div>
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-slate-100">
                  {customers.filter(c => filterSearch(c.name || c.email)).map(customer => (
                    <tr key={customer.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider ${customer.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                  ...
                  {customers.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-slate-400 font-medium">Chưa có khách hàng nào trong hệ thống.</td>
                    </tr>
                  )}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-slate-100">
                  {orders.map((order: any) => (
                    <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${order.paymentMethod === 'QR' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-700'}`}>
                          {order.paymentMethod}
                        </span>
                      </td>
                      <td className="px-6 py-4">
      ...
      case "DASHBOARD":
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                { label: "Doanh thu (Đã giao)", value: formatVND(stats.revenue || 0), icon: DollarSign, color: "text-emerald-600", bg: "bg-emerald-50" },
                { label: "Tổng Đơn hàng", value: stats.orders || 0, icon: ShoppingCart, color: "text-blue-600", bg: "bg-blue-50" },
                { label: "Khách hàng", value: stats.customers || 0, icon: Users, color: "text-indigo-600", bg: "bg-indigo-50" },
                ...
                return (
                  <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div><p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{stat.label}</p><h3 className="text-2xl font-black text-slate-800 mt-1">{stat.value}</h3></div>
                  </div>
                );
              })}
            </div>
