        ...
        <aside className="hidden lg:block w-64 flex-shrink-0">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_25px_rgb(0,0,0,0.03)] sticky top-28 space-y-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              ...
              {(activeSubCategory || selectedSize || selectedFilterColor || priceRange < MAX_PRICE || isSearchActive || activeHeaderTab === "GIẢM GIÁ" || activeHeaderTab !== "TẤT CẢ" || quickFilter !== "ALL") && (
                <button
                  onClick={() => {
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="lg:hidden flex items-center justify-between gap-2.5 mb-6 bg-white p-3 rounded-2xl border border-slate-100 shadow-xs">
            ...
            >
              ...
              <span>Bộ lọc</span>
              {(selectedSize || selectedFilterColor || priceRange < MAX_PRICE || activeSubCategory) && (
                <span className="w-4 h-4 bg-teal-700 text-white rounded-full text-[9px] flex items-center justify-center font-black">
                  {Number(Boolean(selectedSize)) + Number(Boolean(selectedFilterColor)) + Number(priceRange < MAX_PRICE) + Number(Boolean(activeSubCategory))}
                </span>
              )}
            </button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row shadow-2xl relative">
            ...
            <div className="w-full md:w-1/2 p-8 lg:p-10 flex flex-col bg-white">
              ...
              {/* GIÁ ĐỘNG THEO MÀU */}
              {hasDiscount ? (
                <div className="mb-5 p-4 bg-gradient-to-r from-red-50/80 to-rose-50/40 rounded-2xl border border-red-100/70">
                  ...
                  <p className="text-xs font-semibold text-red-700/90 mt-1.5 flex items-center gap-1.5">
                    <span>🔥 Tiết kiệm:</span>
                    <span className="font-bold">{formatVND(displayPrice - displayDiscount)}</span>
                  </p>
                </div>
              ) : (
                <div className="text-2xl sm:text-3xl font-black text-slate-900 mb-5 tracking-tight leading-none">
                  {formatVND(displayPrice)}
                </div>
              )}
