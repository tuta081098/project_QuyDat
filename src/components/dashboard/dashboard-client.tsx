  ...
  return (
    <div className="space-y-8 relative">
          ...
          {overdueProjects.map((p: any) => (
            <div key={p.id} className="pointer-events-auto animate-in slide-in-from-right-full bg-white border-l-4 border-red-600 shadow-2xl p-4 pr-10 rounded-xl flex items-start gap-3 ring-1 ring-black/5 relative group">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1 text-[10px] text-slate-500 mt-1"><Clock className="w-3 h-3" /><span>{T.NOTIFICATIONS.DEADLINE_LABEL} {new Date(p.deadline).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}</span></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button onClick={() => { setFormDetails({ ...defaultDetails, deadline: "" }); setIsCreateModalOpen(true); }} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition-all shadow-md hover:shadow-lg">
          <Plus className="w-5 h-5" /> {T.HEADER.BTN_ADD_NEW}
        </button>
      </div>

      {/* METRICS CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card onClick={() => setActiveFilter('ALL')} className={`cursor-pointer transition-all hover:scale-[102%] border-0 shadow-lg ${activeFilter === 'ALL' ? 'ring-4 ring-slate-800/30' : ''} bg-gradient-to-br from-slate-800 to-slate-900 text-white`}><CardHeader className="pb-2 flex flex-row items-center justify-between"><CardTitle className="text-sm font-medium opacity-80">{T.METRICS.TOTAL_PROJECTS}</CardTitle><Building2 className="h-4 w-4 opacity-70" /></CardHeader><CardContent><div className="text-4xl font-bold">{totalProjects}</div></CardContent></Card>
        <Card onClick={() => setActiveFilter('CHUA_BAN_HANH')} className={`cursor-pointer transition-all hover:scale-[102%] shadow-sm ${activeFilter === 'CHUA_BAN_HANH' ? 'ring-2 ring-slate-400 bg-slate-50' : ''}`}><CardHeader className="pb-2 flex flex-row items-center justify-between"><CardTitle className="text-sm font-medium text-slate-500">{T.METRICS.CHUA_BAN_HANH}</CardTitle><FileWarning className="h-4 w-4 text-slate-400" /></CardHeader><CardContent><div className="text-3xl font-bold text-slate-700">{totalChuaBanHanh}</div></CardContent></Card>
        <Card onClick={() => setActiveFilter('CHUA_KIEM_KE')} className={`cursor-pointer transition-all hover:scale-[102%] shadow-sm ${activeFilter === 'CHUA_KIEM_KE' ? 'ring-2 ring-blue-400 bg-blue-50/50' : ''}`}><CardHeader className="pb-2 flex flex-row items-center justify-between"><CardTitle className="text-sm font-medium text-slate-500">{T.METRICS.CHUA_KIEM_KE}</CardTitle><ClipboardList className="h-4 w-4 text-blue-400" /></CardHeader><CardContent><div className="text-3xl font-bold text-blue-600">{totalChuaKiemKe}</div></CardContent></Card>
        <Card onClick={() => setActiveFilter('DANG_GIAI_QUYET')} className={`cursor-pointer transition-all hover:scale-[102%] shadow-sm ${activeFilter === 'DANG_GIAI_QUYET' ? 'ring-2 ring-orange-400 bg-orange-50/50' : ''}`}><CardHeader className="pb-2 flex flex-row items-center justify-between"><CardTitle className="text-sm font-medium text-slate-500">{T.METRICS.DANG_GIAI_QUYET}</CardTitle><Clock className="h-4 w-4 text-orange-400" /></CardHeader><CardContent><div className="text-3xl font-bold text-orange-600">{totalDangGiaiQuyet}</div></CardContent></Card>
      </div>

      {/* PROJECT GRID */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {sortedProjects.length === 0 && <div className="col-span-full py-12 flex flex-col items-center justify-center text-slate-400 bg-slate-50 border-2 border-dashed rounded-2xl"><FileWarning className="w-10 h-10 mb-2 opacity-50" /><p>{T.PROJECT_CARD.EMPTY_STATE}</p></div>}
        ...
        {sortedProjects.map((project: any) => {
          ...
          return (
            <Card key={project.id} className={`group hover:shadow-lg transition-all duration-300 cursor-pointer overflow-hidden flex flex-col border-2 ${isCompleted ? 'border-green-300 bg-green-50/30' : isOverdue ? 'border-red-400 bg-red-50/20' : 'border-transparent hover:border-blue-400'}`} onClick={() => handleOpenModal(project)}>
              <div className={`h-1.5 w-full transition-opacity ${isCompleted ? 'bg-green-500 opacity-100' : isOverdue ? 'bg-red-500 opacity-100' : 'bg-gradient-to-r from-blue-400 to-indigo-500 opacity-0 group-hover:opacity-100'}`} />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 mb-1.5">
                      ...
                      <span className={`text-xs font-semibold ${isOverdue ? 'text-red-600' : 'text-slate-500'}`}>
                        {T.PROJECT_CARD.DEADLINE_PREFIX} {project.deadline ? new Date(project.deadline).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' }) : T.PROJECT_CARD.NO_DEADLINE}
                      </span>
                      {isOverdue && <Badge className="bg-red-600 text-white text-[10px] py-0 px-1.5 border-0 animate-bounce shadow-md"><AlertTriangle className="w-2.5 h-2.5 mr-1" /> {T.PROJECT_CARD.OVERDUE_BADGE}</Badge>}
                    </div>
                    <p className="text-sm text-slate-500 mt-1">{T.PROJECT_CARD.CATEGORY}: <span className="font-bold text-slate-700">{project.details?.tienDoDuAn || T.PROJECT_CARD.NO_CATEGORY}</span></p>
                  </div>
                  {isCompleted && <Badge className="bg-green-100 text-green-700 whitespace-nowrap"><CheckCircle2 className="w-3 h-3 mr-1" /> {T.PROJECT_CARD.COMPLETED}</Badge>}
                </div>
                {!isCompleted && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {project.chuaBanHanhCount > 0 && <Badge variant="outline" className="bg-slate-50 text-slate-600 border-slate-200">{T.FIELDS.CHUA_BAN_HANH}</Badge>}
                    {project.chuaKiemKeCount > 0 && <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-200">{T.FIELDS.CHUA_KIEM_KE}</Badge>}
                    {project.xacNhanCount > 0 && <Badge variant="outline" className="bg-orange-50 text-orange-600 border-orange-200">{T.FIELDS.XAC_NHAN}</Badge>}
                    {project.duThaoCount > 0 && <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-200">{T.FIELDS.DU_THAO}</Badge>}
                    {project.thamDinhCount > 0 && <Badge variant="outline" className="bg-purple-50 text-purple-600 border-purple-200">{T.FIELDS.THAM_DINH}</Badge>}
                    {project.pheDuyetCount > 0 && <Badge variant="outline" className="bg-green-50 text-green-600 border-green-200">{T.FIELDS.PHE_DUYET}</Badge>}
                  </div>
                )}
              </CardHeader>
              <CardContent className="pt-4 flex-1">
                <div className="mb-6 space-y-2">
                  <div className="flex justify-between text-xs font-medium text-slate-500"><span>{T.PROJECT_CARD.PROGRESS_TITLE}</span><span className="font-bold text-slate-700">{progressPercent}% ({project.pheDuyetCount}/{project.totalLots} {T.PROJECT_CARD.UNIT_LOT})</span></div>
                  <Progress value={progressPercent} className={`h-2 ${isCompleted ? 'bg-green-200' : isOverdue ? 'bg-red-200' : ''}`} />
                </div>
              </CardContent>
            </Card>
          );
                      ...
                      <div key={hist.id} className="relative flex items-start justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm z-10 text-sm ml-8 transition-all hover:shadow-md hover:border-slate-300">
                        ...
                        <div className="flex flex-col gap-2 w-full">
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                            {new Date(hist.createdAt).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}
                          </span>
                          <div className="text-slate-800 leading-relaxed font-medium">{hist.note.split(' | ').map((line: string, i: number) => (<div key={i} className="mb-1">• {line}</div>))}</div></div>
                      </div>
                    ))
