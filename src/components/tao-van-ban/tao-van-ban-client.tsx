...
export default function TaoVanBanClient() {
  ...
  // 4. STATES MODAL LỊCH SỬ & XEM DATA
    ...
    if (file && file.name.endsWith('.docx')) {
      ...
      try {
        ...
        if (extractedKeys.size === 0) {
          showToast("Không tìm thấy từ khóa {key} nào trong file Word", "error");
          setIsLoadingDB(false);
          return;
        }
        ...
        if (res.ok) {
          ...
          showToast("Đã tải mẫu lên hệ thống", "success");
        }
      } catch {
        showToast("Lỗi định dạng file Word", "error");
      } finally {
        setIsLoadingDB(false);
      }
    }
  ...
  const removeTemplate = async (id: string) => {
    if (confirm("Xác nhận xóa mẫu văn bản này? Lịch sử cũng sẽ bị xóa vĩnh viễn.")) {
      setTemplates(templates.filter(t => t.id !== id));
      await fetch(`/api/tao-van-ban?id=${id}`, { method: 'DELETE' });
      showToast("Đã xóa mẫu thành công", "info");
    }
  };
  ...
  // ==========================================
  // XỬ LÝ GHÉP DỮ LIỆU & XUẤT FILE
  // ==========================================
  
  // HÀM MỞ MODAL ĐANG BỊ LỖI ĐÃ ĐƯỢC FIX Ở ĐÂY
  const openUseModal = (template: TemplateItem) => {
    setActiveTemplate(template);
    setExcelFile(null);
    setExcelData([]);
    setExcelColumns([]);
    setNamingTemplate("VanBan_{index}");
    setIsModalOpen(true);
  };
  ...
  const handleExcelSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (jsonData.length > 0) {
        ...
        // Tự động nhận diện cột Tên nếu có
        const autoCol = cols.find(c => ['hoten', 'họ tên', 'tên', 'name', 'ho_ten'].includes(c.toLowerCase()));
        if (autoCol) {
          setNamingTemplate(`VanBan_{${autoCol}}`);
        } else {
          setNamingTemplate("VanBan_{index}");
        }
      }
    } catch {
      showToast("Lỗi đọc dữ liệu file Excel", "error");
    }
  };

  const resolveFileName = (pattern: string, row: any, index: number) => {
    if (!pattern) return `VanBan_${index + 1}`;
    
    let result = pattern.replace(/\{([^}]+)\}/g, (match, key) => {
      const cleanKey = key.trim();
      if (cleanKey.toLowerCase() === 'index') return String(index + 1);
      return row[cleanKey] !== undefined ? String(row[cleanKey]) : "";
    });
    
    // Loại bỏ ký tự cấm trong tên file của Windows/Mac
    ...
    try {
      ...
      excelData.forEach((row, index) => {
        ...
        const doc = new Docxtemplater(zip, { 
          paragraphLoop: true, 
          linebreaks: true, 
          nullGetter() { return ""; } // Để trắng nếu Excel không có dữ liệu
        });
        ...
        // Cấp tên cho từng file con nằm trong ZIP
      ...
      fetchTemplates(); // Cập nhật lại số đếm lịch sử
      setIsModalOpen(false);
      showToast("Đã kết xuất thành công!", "success");
    } catch {
      showToast("Có lỗi trong quá trình trộn văn bản", "error");
    } finally {
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border w-full max-w-md animate-in fade-in zoom-in-95">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="relative">
              ...
              <input type="text" required value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Tài khoản" className="w-full pl-11 p-4 border border-slate-100 rounded-2xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-slate-800 transition-all outline-none" />
            </div>
            <div className="relative">
              <LockKeyhole className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mật khẩu" className="w-full pl-11 p-4 border border-slate-100 rounded-2xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-slate-800 transition-all outline-none" />
            </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
      ...
      {/* Toast Notification */}
      {toast && (
        <div className={`fixed top-6 right-6 px-5 py-3 rounded-2xl shadow-2xl border flex items-center gap-3 z-[100] animate-in slide-in-from-right-full ${toast.type === 'error' ? 'bg-red-50 text-red-700 border-red-100' : 'bg-slate-900 text-white border-slate-800'}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Hệ thống tạo văn bản hàng loạt tự động</p>
        </div>
        <div className="flex gap-3">
          <input type="file" ref={wordInputRef} onChange={handleWordSelect} accept=".docx" className="hidden" />
          <button onClick={() => wordInputRef.current?.click()} className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold text-sm flex items-center gap-2 hover:scale-105 transition-all shadow-md">
            <PlusCircle className="w-5 h-5" /> Tải Lên Mẫu
          </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {templates.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-24 text-center text-slate-300 font-bold italic">
                    {isLoadingDB ? 'Đang đồng bộ dữ liệu...' : 'Chưa có mẫu nào trên hệ thống'}
                  </td>
                </tr>
              ) : (
                templates.map((t, i) => (
                  <tr key={t.id} className="group hover:bg-slate-50/50 transition-all">
                    ...
                    <td className="px-6 py-5 font-bold text-slate-800">{t.name}</td>
                    <td className="px-6 py-5 text-center">
                      <button 
                        onClick={() => {setHistoryTemplate(t); setIsHistoryModalOpen(true);}} 
                        className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-xl text-[11px] font-black group-hover:bg-blue-600 group-hover:text-white transition-all"
                      >
                        {t.histories?.length || 0} lần
                      </button>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <button 
                          ...
                          className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-emerald-700 shadow-sm flex items-center gap-2 transition-all active:scale-95"
                        >
                          <PlayCircle className="w-4 h-4"/> Ghép dữ liệu
                        </button>
                        <button 
          ...
          <div className="bg-white rounded-[32px] w-full max-w-xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
              <h2 className="font-black text-slate-900 flex items-center gap-2 uppercase text-sm tracking-tighter">
                <Settings className="w-5 h-5" /> Cấu hình kết xuất
              </h2>
            ...
            <div className="p-8 space-y-6">
              <div 
                ...
                className={`border-2 border-dashed rounded-3xl p-10 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${excelFile ? 'border-emerald-200 bg-emerald-50/30 scale-[0.98]' : 'border-slate-100 hover:border-slate-300 bg-slate-50/50'}`}
              >
                {excelFile ? (
                  <>
                    <FileSpreadsheet className="w-12 h-12 text-emerald-600 mb-3" />
                    <p className="font-bold text-slate-900">{excelFile.name}</p>
                    <p className="text-xs text-emerald-600 font-bold mt-1">Dữ liệu: {excelData.length} bản ghi</p>
                  </>
                ) : (
                  <>
                    <Upload className="w-10 h-10 text-slate-200 mb-3" />
                    <p className="font-bold text-slate-400 text-sm">Nạp file dữ liệu Excel (.xlsx)</p>
                  </>
                )}
              </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="Ví dụ: vanban_{HO_TEN}_so_{index}" 
                      className="w-full p-4 border-2 border-slate-50 rounded-2xl focus:border-slate-900 focus:bg-white bg-slate-50 outline-none font-bold text-sm transition-all" 
                    />
                    <div className="mt-3 p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                      ...
                      <p className="text-xs font-mono text-blue-700 font-bold break-all">
                        {resolveFileName(namingTemplate, excelData[0], 0)}.docx
                      </p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase mb-2">Biến hỗ trợ:</p>
            ...
            <div className="p-6 bg-slate-50/50 flex gap-3">
              ...
              <button 
                ...
                className="flex-[2] py-4 bg-slate-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-xl shadow-slate-200 disabled:opacity-50 transition-all active:scale-95"
              >
                {isGenerating ? <Loader2 className="w-5 h-5 animate-spin" /> : <FileArchive className="w-5 h-5" />} Tải file ZIP
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[32px] w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center">
              <h2 className="font-black text-slate-900 uppercase text-xs tracking-widest">Lịch sử xuất: {historyTemplate.name}</h2>
              <button onClick={() => setIsHistoryModalOpen(false)}><X className="w-6 h-6 text-slate-300" /></button>
            </div>
            <div className="overflow-y-auto flex-1 p-0">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 sticky top-0 border-b text-[10px] font-black uppercase text-slate-400">
                  <tr>
                    <th className="px-6 py-4">Thời gian</th>
                    <th className="px-6 py-4">Nguồn dữ liệu Excel</th>
                    <th className="px-6 py-4 text-center">Số lượng VB</th>
                    <th className="px-6 py-4 text-right">Thao tác</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {historyTemplate.histories.length === 0 ? (
                    <tr><td colSpan={4} className="py-10 text-center text-slate-400 font-bold text-xs">Mẫu này chưa được xuất lần nào</td></tr>
                  ) : (
                    historyTemplate.histories.map((h) => (
                      <tr key={h.id} className="hover:bg-slate-50/50 transition-all">
                        <td className="px-6 py-5 font-bold text-slate-500">{new Date(h.createdAt).toLocaleString('vi-VN')}</td>
                        <td className="px-6 py-5 font-bold text-slate-800 truncate max-w-[250px]">{h.excelName}</td>
                        <td className="px-6 py-5 text-center">
                          <span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-full font-black text-[10px]">{h.recordCount} file</span>
                        </td>
                        <td className="px-6 py-5 text-right">
                          <button 
                            onClick={() => setPreviewData(h.dataSnapshot!)} 
                            className="text-[10px] font-black uppercase border-2 border-slate-100 px-3 py-1.5 rounded-xl hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all"
                          >
                            Dữ liệu
                          </button>
                        </td>
      ...
      {/* MODAL 3: XEM TRƯỚC DỮ LIỆU ĐÃ ĐẨY */}
      {previewData && (
          ...
          <div className="bg-white rounded-[40px] w-full max-w-6xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
            <div className="p-6 border-b bg-slate-900 text-white flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-black uppercase text-xs tracking-widest">Dữ liệu lưu trữ</h2>
              </div>
              <button onClick={() => setPreviewData(null)}><X className="w-6 h-6 text-slate-500 hover:text-white" /></button>
            </div>
            <div className="overflow-auto flex-1">
              <table className="w-full text-left text-[11px] whitespace-nowrap border-collapse">
                <thead className="bg-slate-50 sticky top-0 border-b shadow-sm">
                  <tr>
                    <th className="px-5 py-4 font-black text-slate-400 bg-slate-50 text-center w-12 border-r">#</th>
                    {Object.keys(previewData[0] || {}).map(k => (
                      <th key={k} className="px-5 py-4 font-black text-slate-600 bg-slate-50 border-r">{k}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {previewData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/30 transition-all">
                      <td className="px-5 py-3 border-r text-center text-slate-300 font-bold">{idx + 1}</td>
                      {Object.keys(previewData[0] || {}).map(k => (
                        <td key={k} className="px-5 py-3 border-r text-slate-600 font-medium">
