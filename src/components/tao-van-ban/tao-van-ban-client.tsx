...
export default function TaoVanBanClient() {
  ...
  // ===================================================================
  // HÀM ĐỌC EXCEL - ĐÃ FIX LỖI HOÁN ĐỔI DẤU CHẤM PHẨY THEO CHUẨN VN
  // ===================================================================
    ...
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        sheetData.forEach((rowObj: any, index: number) => {
          ...
          // ---- BẮT ĐẦU FIX FORMAT SỐ LIỆU ----
          const fixedRowObj: any = {};
          for (const key in rowObj) {
            let val = rowObj[key];
            if (typeof val === 'string') {
              const trimmed = val.trim();
              
              // Regex cực kỳ chặt chẽ: Nhận dạng số liệu format US (VD: 1,234.56 hoặc 1000.5) 
          ...
          // ---- KẾT THÚC FIX FORMAT SỐ LIỆU ----

          if (!mergedData[index]) mergedData[index] = {};
          mergedData[index] = { ...mergedData[index], ...fixedRowObj };
        });
      });
  ...
  const removeTemplate = async (id: string) => {
    if (confirm("Chuyển mẫu này vào thùng rác? Bạn có thể khôi phục từ DB sau.")) {
      setTemplates(templates.filter(t => t.id !== id));
      await fetch(`/api/tao-van-ban?id=${id}`, { method: 'DELETE' });
      showToast("Đã đưa vào thùng rác", "info");
    }
  };
  ...
  const ToastComponent = toast && (
    <div className={`fixed top-6 right-6 px-6 py-4 rounded-2xl shadow-2xl border flex items-center justify-between gap-4 z-[9999] animate-in slide-in-from-right-full duration-300 ${
