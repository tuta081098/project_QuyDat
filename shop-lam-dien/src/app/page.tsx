...
import { formatVND, formatCurrencyInput, parseCurrency } from "@/src/lib/format";
  ...
  useEffect(() => {
    ...
    const loadShopData = async () => {
      ...
      try {
        ...
        if (prodRes.ok) setProducts((await prodRes.json()).filter((p: any) => p.status === "ACTIVE"));
        ...
        <aside className="hidden lg:block w-64 flex-shrink-0">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_25px_rgb(0,0,0,0.03)] sticky top-28 space-y-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              ...
              {(activeSubCategory || selectedSize || priceRange < MAX_PRICE || isSearchActive || activeHeaderTab === "GIẢM GIÁ" || activeHeaderTab !== "TẤT CẢ" || quickFilter !== "ALL") && (
                <button
                  onClick={() => {
              ...
              <div className="mt-3">
                <div className="relative">
                  <input
                    type="text"
                    inputMode="numeric"
                    placeholder="Nhập mức giá lọc..."
                    value={priceRange >= MAX_PRICE ? "" : formatCurrencyInput(priceRange)}
                    onChange={(e) => {
                      const val = parseCurrency(e.target.value);
                      setPriceRange(val > 0 ? Math.min(val, MAX_PRICE) : MAX_PRICE);
                    }}
                    className="w-full pl-3 pr-8 py-2 text-xs font-bold border border-slate-200 rounded-lg bg-white outline-none focus:border-teal-600 transition-colors"
                  />
                ...
                {priceRange < MAX_PRICE && (
                  <button
                    ...
                    className="text-[10px] text-teal-700 hover:text-teal-900 font-bold mt-1.5 inline-block"
                  >
                    ↺ Đặt lại tất cả mức giá
                  </button>
                )}
              </div>
            </div>
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="lg:hidden flex items-center justify-between gap-2.5 mb-6 bg-white p-3 rounded-2xl border border-slate-100 shadow-xs">
            ...
            >
              ...
              <span>Bộ lọc</span>
              {(selectedSize || priceRange < MAX_PRICE || activeSubCategory) && (
                <span className="w-4 h-4 bg-teal-700 text-white rounded-full text-[9px] flex items-center justify-center font-black">
                  {Number(Boolean(selectedSize)) + Number(priceRange < MAX_PRICE) + Number(Boolean(activeSubCategory))}
                </span>
              )}
            </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-5 mb-6 border-b border-slate-100 gap-3">
            ...
            {(activeSubCategory || selectedSize || priceRange < MAX_PRICE || isSearchActive || quickFilter !== "ALL") && (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {priceRange < MAX_PRICE && (
                  <span className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 text-xs font-bold flex items-center gap-1.5">
                    ...
                    <button onClick={() => setPriceRange(MAX_PRICE)} className="hover:text-red-500 p-0.5" title="Bỏ lọc giá">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
            ...
            <div className="text-center py-20 bg-white border border-slate-100 shadow-sm rounded-3xl p-8">
              ...
              {(activeSubCategory || selectedSize || priceRange < MAX_PRICE || isSearchActive || quickFilter !== "ALL") ? (
                <>
                  ...
                  <button
                    onClick={() => { setActiveSubCategory(""); setSelectedSize(""); setPriceRange(MAX_PRICE); handleClearSearch(); setQuickFilter("ALL"); setIsSearchOpen(false); }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-4 sm:p-5 flex flex-col flex-1">
                        ...
                        {/* Price Display */}
                        <div className="mt-auto mb-3.5 min-h-[44px] flex flex-col justify-end">
                          {isSale ? (
                            <div className="flex flex-col gap-0.5">
                              <div className="flex items-baseline gap-1.5 flex-wrap">
                                <span className="text-red-600 font-black text-base sm:text-lg leading-tight tracking-tight whitespace-nowrap">
                                  {formatVND(prod.discountPrice)}
                                </span>
                                <span className="text-[10px] font-black text-red-600 bg-red-50 border border-red-100/80 px-1.5 py-0.5 rounded leading-none">
                                  -{percentOff}%
                                </span>
                              </div>
                              <span className="text-slate-400 line-through text-xs font-semibold leading-none truncate">
                                {formatVND(prod.price)}
                              </span>
                            </div>
                          ) : (
                            <div className="flex flex-col gap-0.5">
                              <span className="text-slate-900 font-black text-base sm:text-lg leading-tight tracking-tight whitespace-nowrap">
                                {formatVND(prod.price)}
                              </span>
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div>
                ...
                {/* Ô nhập giá trực tiếp trên mobile */}
                <div className="mt-3">
                  <div className="relative">
                    <input
                      type="text"
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row shadow-2xl relative">
            ...
            <div className="w-full md:w-1/2 p-8 lg:p-10 flex flex-col bg-white">
              ...
              {selectedProduct.discountPrice && selectedProduct.discountPrice > 0 ? (
                <div className="mb-6 p-4 bg-gradient-to-r from-red-50/80 to-rose-50/40 rounded-2xl border border-red-100/70">
                  <div className="flex flex-wrap items-baseline gap-2.5 sm:gap-3">
                    <span className="text-2xl sm:text-3xl font-black text-red-600 tracking-tight leading-none">
                      {formatVND(selectedProduct.discountPrice)}
                    </span>
                    <span className="text-sm sm:text-base text-slate-400 line-through font-semibold">
                      {formatVND(selectedProduct.price)}
                    </span>
                    <span className="bg-gradient-to-r from-red-500 to-rose-600 text-white text-[11px] font-black px-2 py-0.5 rounded-md uppercase shadow-sm">
                      -{Math.round(((selectedProduct.price - selectedProduct.discountPrice) / selectedProduct.price) * 100)}%
                    </span>
                  </div>
                  <p className="text-xs font-semibold text-red-700/90 mt-1.5 flex items-center gap-1.5">
                    <span>🔥 Tiết kiệm:</span>
                    <span className="font-bold">{formatVND(selectedProduct.price - selectedProduct.discountPrice)}</span>
                  </p>
                </div>
              ) : (
                <div className="text-2xl sm:text-3xl font-black text-slate-900 mb-6 tracking-tight leading-none">
                  {formatVND(selectedProduct.price)}
                </div>
              )}
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div className="flex items-baseline gap-1.5">
                          <span className="font-black text-teal-700 text-sm">{formatVND(item.price * item.quantity)}</span>
                          {item.quantity > 1 && (
                            <span className="text-[10px] text-slate-400 font-medium">({formatVND(item.price)}/sp)</span>
                          )}
                        </div>
